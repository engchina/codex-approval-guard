...
export function buildReleaseBody({
  ...
  const sections = []
  const draftBody = releaseNotesDraftSource
    ? extractReleaseNotesDraftBody(releaseNotesDraftSource)
    : null
...
export async function loadReleaseBody({
  ...
  let releaseNotesDraftSource = null
  try {
    releaseNotesDraftSource = await readFile(path.join(cwd, RELEASE_NOTES_DRAFT_PATH), 'utf8')
  } catch (error) {
...
if (isDirectExecution()) {
  ...
  if (!version) {
    console.error('Expected a version argument such as "0.2.0" or GITHUB_REF_NAME like "v0.2.0".')
    process.exitCode = 1
  } else {
    try {
